...
function TabbedGUI()
   ...
   statusLabel = uilabel(tab22, ...
        ...
        'Position', [300 490 300 25],FontSize=18, FontName='Arial',FontWeight='bold');
    % p = uipanel(tab22);
 ...
 uilabel(tab22,"Text",'Resulted Plots',Position=[350,300,400,50],FontSize=20,FontName='Amasis MT Pro Black',...
        FontWeight = 'bold');
  

 %% Tab12  --------------------------------------
  pathFieldsrc12 = uieditfield(tab12, 'text', ...
        'Position', [20 520 440 30], ...
        'Placeholder', 'Filtered Data Files Path...');

    browseBtnsrc12 = uibutton(tab12, ...
        'Text', 'Browse...', ...
        'Position', [470 520 100 30], ...
        'ButtonPushedFcn', @(btn,event) browseFile('src'));
    lstbx121= uilistbox(tab12,Position=[20,380,790,100],FontColor='#1A873A',Items={},...
        ValueChangedFcn= @(src,event) getlstbxitem(src));

      uilabel(tab12, ...
        'Text', 'List of Filtered Data Files',  ...
        'Position', [300 480 300 25],FontSize=18, FontName='Arial',FontWeight='bold');
    % p = uipanel(tab22);
    % p.Title = 'Image Editor';
    % p.Position = [100,5,650,310];
    % resimg= uiimage(p,"ImageSource",'BaselinePrelimFindPeaks.jpg', Position=[50,10,300,280]);
    resimg= uiimage(tab12,"ImageSource",'BaselinePrelimFindPeaks.jpg', Position=[300,5,240,240]);
